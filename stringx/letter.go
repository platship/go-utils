...
func FirstLetter(val string) string {
	// 获取中文字符串第一个字符
	firstChar := string([]rune(val)[:1])
	firstCharGbk, err := Utf8ToGbk([]byte(firstChar)) // Utf8 转 GBK2312
	if err != nil {
		return ""
	}
	// 如果是一个字节，就直接返回
	if len(firstCharGbk) == 1 {
		return strings.ToUpper(firstChar) // 所有转大写
	}
	return LetterByByte(firstCharGbk)
}

func FirstLetterAll(val string) (ret string) {
	valArr := []rune(val)
	for _, v := range valArr {
		charGbk, err := Utf8ToGbk([]byte(string(v)))
		if err == nil {
			if len(charGbk) == 1 {
				ret += strings.ToUpper(string(charGbk))
			} else {
				ret += LetterByByte(charGbk)
			}
...
func LetterByByte(char []byte) string {
	// 获取第一个字符的16进制
	firstCharHex := hex.EncodeToString(char)
